      ...
      const fetchDynamicPlaceholders = async () => {
          try {
              ...
              const response = await ai.models.generateContent({
                  ...
                  contents: 'Generate 20 creative, short, diverse UI component prompts (e.g. "bioluminescent task list"). IP SAFEGUARD: Avoid referencing specific famous artists, movies, or brands.',
                  config: {
              ...
              try {
                  const newPlaceholders = JSON.parse(text);
                  if (Array.isArray(newPlaceholders) && newPlaceholders.length > 0) {
                      const shuffled = newPlaceholders.sort(() => 0.5 - Math.random()).slice(0, 10);
                      setPlaceholders(prev => [...prev, ...shuffled]);
                  }
              } catch (e) {
                  console.warn("Failed to parse dynamic placeholders", e);
              }
          } catch (e) {
...
- Generate high-fidelity HTML/CSS.
        ...
        const responseStream = await ai.models.generateContentStream({
            ...
            config: { 
                temperature: 1.2,
                systemInstruction: "You are a master UI/UX designer. Output your response as a JSON array of objects, each containing a 'name' and 'html' property.",
                responseMimeType: "application/json",
                responseSchema: {
                    type: Type.ARRAY,
                    items: {
...
- Example D: "Spectral Prismatic Diffusion" (Glassmorphism, caustic refraction, soft-focus morphing gradients).
        ...
        const styleResponse = await ai.models.generateContent({
            ...
            config: {
                systemInstruction: "You are a creative director. Return exactly 3 creative names for design directions.",
                responseMimeType: "application/json",
                responseSchema: {
                    type: Type.ARRAY,
                    items: { type: Type.STRING }
        ...
        try {
            generatedStyles = JSON.parse(styleResponse.text || '[]');
        } catch (e) {
...
5. **Layout**: Be bold with negative space and hierarchy. Avoid generic cards.
          `.trim();
                ...
                const responseStream = await ai.models.generateContentStream({
                    ...
                    contents: prompt,
                    config: {
                        systemInstruction: "You are Flash UI, an expert frontend developer and UI/UX designer. Return ONLY RAW HTML with embedded CSS/JS. Do not use markdown code blocks or formatting.",
                    }
                });
